...
async def test_terminology_validate_code_endpoint_fails_closed(
    client: httpx.AsyncClient, mock_http: respx.MockRouter
) -> None:
    """An unanswered question must never be reported as ``result: false``."""
...
async def test_terminology_operation_outcome_is_not_a_negative_answer(
    client: httpx.AsyncClient, mock_http: respx.MockRouter
) -> None:
    """Some servers answer errors with an OperationOutcome and HTTP 200."""
    mock_http.post(f"{TERMINOLOGY_URL}/CodeSystem/$validate-code").mock(
        return_value=fhir_json(
            {
                "resourceType": "OperationOutcome",
                "issue": [
                    {"severity": "error", "code": "processing", "diagnostics": "index unavailable"}
                ],
            }
        )
    )
...
async def test_unknown_valueset_is_a_client_error_not_an_outage(
    client: httpx.AsyncClient, mock_http: respx.MockRouter
) -> None:
    """404 means "the server does not know that ValueSet", which the caller can fix.

    Conflating it with an outage would tell an operator to page someone.
    """
    ...
    response = await client.post(
        ...
        json={
            ...
            "code": "8867-4",
            "value_set": "http://example.org/ValueSet/does-not-exist",
        },
    )

    assert response.status_code == 422
    assert response.json()["issue"][0]["details"]["coding"][0]["code"] == "unknown-value-set"
